	...
	March 2014
-------------------------------
ExportMnf is a simple Windows command line application that loads and exports
...
Simple usage is just:
     ...
     exportmnf d:\eso\somepath\data\game.mnf d:\esoexport\
...
You can try to convert DDS files to PNG automatically by:
    ...
    exportmnf game.mnf d:\esoexport\ -c
...
There are several more advanced command line options which may be useful:
	...
	exportmnf eso.mnf .\export\ -a 123
	    Only export a specific archive index (i.e., all files from
	    eso0123.dat).

	exportmnf eso.mnf .\export\ -f 1234
	...
	exportmnf eso.mnf .\export\ -z zosft.txt
	    Save the ZOS file table (if found) to the given filename in a CSV
		format.

	exportmnf eso.mnf .\export\ -m mnfft.txt
...
-------------------------------
   ...
   - One known issue is that there are multiple files from eso0000.dat which
     cannot be uncompressed due to having an unknown format.