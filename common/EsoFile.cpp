...
namespace eso {
	...
	fpos_t CFile::GetSize (void)
	{
		...
		if (m_pFile == nullptr) 
		{
			PrintDebug("Error: File not open in CFile::GetSize()!");
			return 0;
		}

		if (fgetpos(m_pFile, &CurPos) != 0) 
		{
			PrintDebug("Error: Failed to get current position in file!");
			return 0;
		}
		...
		if (fsetpos(m_pFile, &CurPos) != 0) PrintDebug("Error: Failed to reset file position in CFile::GetSize()!");
	...
	bool CFile::Open (std::string Filename, std::string Mode)
	{
		...
		if (m_pFile == nullptr) return PrintDebug("Error: Failed to open file '%s' in mode '%s'!", Filename.c_str(), Mode.c_str());
	...
	bool CFile::Printf (const char* pString, ...)
	{
		...
		if (m_pFile == nullptr) return PrintDebug("Error: No file defined to write to!");
	...
	byte* CFile::ReadAll(fpos_t& FileSize)
	{
		...
		if (m_pFile == nullptr) 
		{
			PrintDebug("Error: No file defined to read from!");
			return nullptr;
		}
		...
		if (FileSize >= INT_MAX)
		{
			PrintDebug("Error: Exceeded maximum size of file that can be loaded at once!");
			return nullptr;
		}
	...
	{
		if (m_pFile == nullptr) return PrintDebug("Error: No file defined to read from!");
		size_t ReadBytes = fread(&Output, 1, sizeof(dword64), m_pFile);
		if (ReadBytes != sizeof(dword64)) return PrintDebug("Error: Only read %u of %u bytes from position 0x%llX!", ReadBytes, sizeof(dword64), Tell());
	...
	bool CFile::ReadFloat (float& Output, const bool LittleEndian)
	{
		...
		if (ReadBytes != sizeof(float)) return PrintDebug("Error: Only read %u of %u bytes from position 0x%llX!", ReadBytes, sizeof(float), Tell());
	...
	bool CFile::ReadDword (dword& Output, const bool LittleEndian)
	{
		...
		if (ReadBytes != sizeof(dword)) return PrintDebug("Error: Only read %u of %u bytes from position 0x%llX!", ReadBytes, sizeof(dword), Tell());
	...
	bool CFile::ReadWord (word&  Output, const bool LittleEndian)
	{
		...
		if (ReadBytes != sizeof(word)) return PrintDebug("Error: Only read %u of %u bytes from position 0x%llX!", ReadBytes, sizeof(word), Tell());
	...
	bool CFile::ReadByte (byte& Output)
	{
		...
		if (ReadBytes != 1) return PrintDebug("Error: Only read %u of %u bytes from position 0x%llX!", ReadBytes, 1, Tell());
		return true;
	}
	...
	bool CFile::ReadBytes (byte* pOutput, const size_t Count)
	{
		...
		if (ReadBytes != Count) return PrintDebug("Error: Only read %u of %u bytes from position 0x%llX!", ReadBytes, Count, Tell());
		return true;
	}
	...
	bool CFile::Seek (const fpos_t Pos, const int SeekType)
	{
		...
		{
			if (fsetpos(m_pFile, &Pos) != 0) return PrintDebug("Error: Failed to seek to position 0x%016llX from start of file!", Pos);
		}
		else if (SeekType == SEEK_END)
		{
			if (_fseeki64(m_pFile, Pos, SEEK_END) != 0) return PrintDebug("Error: Failed to seek to position 0x%016llX from end of file!", Pos);
		}
		else if (SeekType == SEEK_CUR)
		{
			if (_fseeki64(m_pFile, Pos, SEEK_CUR) != 0) return PrintDebug("Error: Failed to seek to position 0x%016llX relative to current file position!", Pos);
		}
		else
		{
			PrintDebug("Error: Unknown seek type %d received in CFile::Seek()!", SeekType);
			return false;
		}
	...
	fpos_t CFile::Tell (void)
	{
		...
		if (fgetpos(m_pFile, &Pos) != 0) PrintDebug("Error: Failed to get current file position!");
		return Pos;
	}
	...
	bool CFile::WriteBytes (const byte* pData, const size_t Size)
	{
		...
		if (WriteBytes != Size) return PrintDebug("Error: Only wrote %u of %u bytes to file!", WriteBytes, Size);
	...
	bool CFile::WriteDword (dword Output, const bool LittleEndian)
	{
		...
		if (WriteBytes != sizeof(dword)) return PrintDebug("Error: Only wrote %u of %u bytes at position 0x%llX!", WriteBytes, sizeof(dword), Tell());
		return true;
	}
	...
	bool CFile::WriteString (const std::string Data)
	{
		...
		if (WriteBytes != Data.length()) return PrintDebug("Error: Only wrote %u of %u bytes at position 0x%llX!", WriteBytes, Data.length(), Tell());
		return true;
	}
